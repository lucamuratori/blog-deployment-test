...
def register():
    ...
    if form.validate_on_submit():
        hashed_password = generate_password_hash(request.form['password'], method='pbkdf2', salt_length=8)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        result = db.session.execute(db.select(User).where(User.email == email_log))
        user = result.scalar()

        if user:
            if check_password_hash(pwhash=user.password, password=password_log):
                login_user(user)
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form, is_edit=True, id=current_user.id)
